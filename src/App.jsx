...
const STG_LBL={requerimiento:"Requerimiento",pago:"Pago",en_proceso:"En Proceso",finalizado:"Finalizado"};
const I={appearance:"none",WebkitAppearance:"none",background:"#fff",border:"1px solid #e5e5e5",borderRadius:7,padding:"8px 11px",fontSize:12,color:"#1a1a1a",width:"100%",fontFamily:"'Sora',sans-serif",outline:"none",transition:"border-color 0.12s",boxSizing:"border-box"};
...
function LoginScreen({onLogin}){
  const users=getAllUsers();const[email,setEmail]=useState("");const[err,setErr]=useState("");
  const go=()=>{const u=users.find(x=>x.email.toLowerCase()===email.trim().toLowerCase());if(u){setCurUser(u);onLogin(u)}else setErr("Correo no autorizado")};
...
function Dashboard({items,onBack}){
  ...
  const gc=(urg,alt)=>urg>0?{bg:"#fef2f2",bd:"#fecaca",tx:"#dc2626"}:alt>0?{bg:"#fff7ed",bd:"#fed7aa",tx:"#ea580c"}:{bg:"#f0fdf4",bd:"#bbf7d0",tx:"#16a34a"};

  const renderGrid=(key,groups)=><div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(270,1fr))",gap:10}}>{groups.map(g=>{
    const its=items.filter(i=>i[key]===g);const op=its.filter(i=>i.stage!=="finalizado");const cl=its.filter(i=>i.stage==="finalizado");
    const urg=op.filter(i=>i.priority==="Urgente").length;const alt=op.filter(i=>i.priority==="Alta").length;
    const med=op.filter(i=>i.priority==="Media").length;const baj=op.filter(i=>i.priority==="Baja").length;
    const c=gc(urg,alt);
    return(<div key={g} style={{background:c.bg,border:"1px solid "+c.bd,borderRadius:10,padding:"14px 16px",transition:"transform 0.1s"}} onMouseEnter={e=>e.currentTarget.style.transform="translateY(-2px)"} onMouseLeave={e=>e.currentTarget.style.transform="none"}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:8}}>
        <div style={{fontSize:13,fontWeight:600}}>{g}</div>
        <div style={{fontSize:18,fontWeight:700,fontFamily:"'JetBrains Mono',monospace",color:c.tx}}>{op.length}</div>
      </div>
      <div style={{display:"flex",gap:3,flexWrap:"wrap",marginBottom:10,minHeight:14}}>
        {op.map((t,i)=>{const pc=PRIORITY[t.priority];return<div key={i} style={{width:10,height:10,borderRadius:2,background:pc.dot}} title={"#"+String(t.num).padStart(3,"0")+" "+t.priority+" - "+t.desc.substring(0,40)}/>})}
        {op.length===0&&<span style={{fontSize:10,color:"#a3a3a3",fontStyle:"italic"}}>Sin tickets abiertos</span>}
      </div>
      <div style={{display:"flex",gap:12,fontSize:10,color:"#737373"}}>
        <span><b style={{color:"#1a1a1a",fontFamily:"'JetBrains Mono',monospace"}}>{its.length}</b> total</span>
        <span><b style={{color:c.tx,fontFamily:"'JetBrains Mono',monospace"}}>{op.length}</b> abiertos</span>
        <span><b style={{color:"#16a34a",fontFamily:"'JetBrains Mono',monospace"}}>{cl.length}</b> cerrados</span>
      </div>
      {(urg>0||alt>0||med>0)&&<div style={{display:"flex",gap:8,marginTop:8,fontSize:10}}>
        {urg>0&&<span style={{color:"#dc2626",fontWeight:600}}>{urg} urgente</span>}
        {alt>0&&<span style={{color:"#ea580c",fontWeight:600}}>{alt} alta</span>}
        {med>0&&<span style={{color:"#d97706"}}>{med} media</span>}
        {baj>0&&<span style={{color:"#a3a3a3"}}>{baj} baja</span>}
      </div>}
    </div>);
  })}</div>;
  ...
  return(
    <div style={{flex:1,overflowY:"auto",padding:"20px 24px"}}>
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:20}}>
        ...
        <div style={{display:"flex",gap:4}}><button onClick={()=>setTab("sedes")} style={ts(tab==="sedes")}>Sedes</button><button onClick={()=>setTab("tipos")} style={ts(tab==="tipos")}>Tipos</button><button onClick={()=>setTab("usuarios")} style={ts(tab==="usuarios")}>Usuarios</button></div>
      </div>
      {tab==="sedes"&&<div><div style={SL}>Vista por sedes ({uniqSedes.length})</div>{renderGrid("sede",uniqSedes)}</div>}
      {tab==="tipos"&&<div><div style={SL}>Vista por tipo</div>{renderGrid("category",catKeys)}</div>}
      {tab==="usuarios"&&<div>
        <div style={SL}>Usuarios registrados ({users.length})</div>
        <div style={{background:"#fafafa",borderRadius:10,border:"1px solid #f0f0f0",overflow:"hidden",marginBottom:16}}>
...
export default function App(){
  ...
  const act=items.filter(i=>i.stage!=="finalizado").length;
  const totC=items.reduce((s,i)=>s+(i.amount||0),0);
  const cls=items.filter(i=>i.closedAt);
  const avg=cls.length?Math.round(cls.map(i=>daysAgo(i.date)-daysAgo(i.closedAt)).reduce((a,b)=>a+b,0)/cls.length):0;
    ...
    <div style={{fontFamily:"'Sora',sans-serif",background:"#fff",color:"#1a1a1a",minHeight:"100vh",fontSize:13,display:"flex",flexDirection:"column"}}>
      ...
      <div style={{padding:"12px 24px",borderBottom:"1px solid #f0f0f0",display:"flex",alignItems:"center",justifyContent:"space-between",flexShrink:0,background:"#fff",position:"sticky",top:0,zIndex:50}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:8}}>
          ...
          {user.role==="admin"&&<button onClick={()=>setShowDash(!showDash)} style={{...showDash?BP:BD,padding:"6px 14px",fontSize:11}}><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke={showDash?"#fff":"currentColor"} strokeWidth="2"><rect x="3" y="3" width="7" height="7"/><rect x="14" y="3" width="7" height="7"/><rect x="3" y="14" width="7" height="7"/><rect x="14" y="14" width="7" height="7"/></svg></button>}
