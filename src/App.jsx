...
function Detail({item,onClose,onUpdate,onDelete,saving,user}){
  ...
  const cPay=user.role==="admin";const cFin=user.role==="ops"||user.role==="admin";const cDel=user.role==="admin";const isAdmin=user.role==="admin";
    ...
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.15)",zIndex:1000,display:"flex",justifyContent:"flex-end",backdropFilter:"blur(3px)",animation:"fadeIn 0.1s ease"}} onClick={onClose}>
      <div style={{width:500,maxWidth:"94vw",background:"#fff",height:"100vh",overflowY:"auto",borderLeft:"1px solid #e5e5e5",animation:"slideIn 0.15s ease",display:"flex",flexDirection:"column"}} onClick={e=>e.stopPropagation()}>
        ...
        <div style={{padding:"16px 24px",borderBottom:"1px solid #f0f0f0",flexShrink:0,display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
          ...
          <div><div style={{fontSize:9,fontWeight:500,color:"#b3b3b3",textTransform:"uppercase",marginBottom:2}}>Solicitante</div>{isAdmin?<div style={{display:"flex",gap:4,alignItems:"center"}}><input value={editBy} onChange={e=>setEditBy(e.target.value)} style={{...I,padding:"4px 8px",fontSize:12,width:"auto",flex:1}}/>{editBy!==item.by&&<button onClick={()=>onUpdate(item.id,{by:editBy})} style={{...BP,padding:"3px 8px",fontSize:10}}>OK</button>}</div>:<div style={{fontSize:12,fontWeight:500}}>{item.by}</div>}</div>
          ...
          <div><div style={{fontSize:9,fontWeight:500,color:"#b3b3b3",textTransform:"uppercase",marginBottom:2}}>Ticket</div><div style={{fontSize:12,fontWeight:500}}>{"#"+String(item.num).padStart(3,"0")}</div></div>
        </div>
...
export default function App(){
  ...
  const filt=items.filter(t=>{if(user.role==="cm"&&t.by!==user.name)return false;if(search){const s=search.toLowerCase();if(!t.desc.toLowerCase().includes(s)&&!t.sede.toLowerCase().includes(s)&&!(t.provider||"").toLowerCase().includes(s)&&!String(t.num).padStart(3,"0").includes(s))return false}if(fCat&&t.category!==fCat)return false;if(fPri&&t.priority!==fPri)return false;return true});
  const selD=sel?items.find(i=>i.id===sel.id):null;
  const myItems=user.role==="cm"?items.filter(i=>i.by===user.name):items;
  const act=myItems.filter(i=>i.stage!=="finalizado").length;const totC=myItems.reduce((s,i)=>s+(i.amount||0),0);
