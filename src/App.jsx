...
const DEMO = [
  ...
  {id:2,num:2,category:"Mantencion",desc:"Cambio rejilla desague",sede:"Salesforce",priority:"Urgente",stage:"en_proceso",by:"Luis Morales",date:"2025-12-03",provider:"Servicios IA SPA",amount:95200,payment:"50",closedAt:null,execDate:"2026-02-20",comments:[],assignee:null},
  {id:3,num:3,category:"Servicio Adicional",desc:"Tope puerta oficinas piso 3",sede:"Abedules",priority:"Baja",stage:"pago",by:"Ana Rondon",date:"2026-02-12",provider:"Servicios IA",amount:71400,payment:null,closedAt:null,execDate:null,comments:[],assignee:null},
  {id:4,num:4,category:"Aire Acondicionado",desc:"Revision gas split oficina 301",sede:"Plaza Egana",priority:"Media",stage:"requerimiento",by:"Vito Lacasella",date:"2026-02-17",provider:null,amount:null,payment:null,closedAt:null,execDate:null,comments:[],assignee:null},
];
...
function Card({item,onOpen,onDragStart}){
  ...
  return(<div draggable onDragStart={e=>{e.dataTransfer.setData("text/plain",String(item.id));onDragStart(item.id)}} onClick={()=>onOpen(item)} style={{background:"#fff",border:"1px solid #ebebeb",borderRadius:8,padding:"10px 12px",cursor:"grab",transition:"box-shadow 0.12s,transform 0.12s",borderLeft:"3px solid "+cat.color}} onMouseEnter={e=>{e.currentTarget.style.boxShadow="0 3px 12px rgba(0,0,0,0.05)";e.currentTarget.style.transform="translateY(-1px)"}} onMouseLeave={e=>{e.currentTarget.style.boxShadow="none";e.currentTarget.style.transform="none"}}>
    ...
    <div style={{display:"flex",alignItems:"center",justifyContent:"space-between"}}><span style={{fontSize:10,color:"#b3b3b3"}}>{item.sede}</span><div style={{display:"flex",alignItems:"center",gap:6}}>{item.assignee&&<span style={{fontSize:9,color:"#3b82f6",fontWeight:500}}>@{item.assignee.split(" ")[0]}</span>}{item.amount!=null&&<span style={{fontSize:10,fontFamily:"'JetBrains Mono',monospace",fontWeight:500,color:"#525252"}}>{fmt(item.amount)}</span>}<span style={{fontSize:9,color:age>14?"#ef4444":age>7?"#f59e0b":"#b3b3b3",fontWeight:age>14?600:400,fontFamily:"'JetBrains Mono',monospace"}}>{age}d</span></div></div>
  </div>);
}
...
function NotifBell({user}){
  const[open,setOpen]=useState(false);const[notifs,setNotifs]=useState(getNotifs());const ref=useRef(null);
  const unread=notifs.filter(n=>!n.read).length;
  useEffect(()=>{const h=e=>{if(ref.current&&!ref.current.contains(e.target))setOpen(false)};document.addEventListener("mousedown",h);return()=>document.removeEventListener("mousedown",h)},[]);
  useEffect(()=>{const iv=setInterval(()=>setNotifs(getNotifs()),5000);return()=>clearInterval(iv)},[]);
  const handleOpen=()=>{if(!open){setOpen(true)}else{setOpen(false)}};
  const handleMarkRead=()=>{markNotifsRead();setNotifs(getNotifs())};
  const mine=notifs.filter(n=>!n.to||n.to===user.name||n.to==="all");
  return(<div ref={ref} style={{position:"relative"}}>
    <button onClick={handleOpen} style={{background:"none",border:"none",cursor:"pointer",padding:4,position:"relative"}}><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke={unread>0?"#f97316":"#b3b3b3"} strokeWidth="2"><path d="M18 8A6 6 0 0 0 6 8c0 7-3 9-3 9h18s-3-2-3-9"/><path d="M13.73 21a2 2 0 0 1-3.46 0"/></svg>{unread>0&&<span style={{position:"absolute",top:0,right:0,width:14,height:14,borderRadius:99,background:"#ef4444",color:"#fff",fontSize:8,fontWeight:700,display:"flex",alignItems:"center",justifyContent:"center"}}>{unread>9?"9+":unread}</span>}</button>
    {open&&<div style={{position:"absolute",top:"100%",right:0,marginTop:8,width:340,maxHeight:400,background:"#fff",border:"1px solid #e5e5e5",borderRadius:10,boxShadow:"0 12px 32px rgba(0,0,0,0.12)",zIndex:100,display:"flex",flexDirection:"column"}}>
      <div style={{padding:"12px 16px",borderBottom:"1px solid #f0f0f0",display:"flex",justifyContent:"space-between",alignItems:"center"}}><span style={{fontSize:13,fontWeight:600}}>Notificaciones</span>{unread>0&&<button onClick={handleMarkRead} style={{fontSize:10,color:"#3b82f6",background:"none",border:"none",cursor:"pointer",fontFamily:"'Sora',sans-serif"}}>Marcar leidas</button>}</div>
      <div style={{flex:1,overflowY:"auto"}}>{mine.length===0?<div style={{padding:24,textAlign:"center",color:"#d4d4d4",fontSize:12}}>Sin notificaciones</div>:mine.slice(0,20).map((n,i)=><div key={i} style={{padding:"10px 16px",borderBottom:"1px solid #f5f5f5",background:n.read?"transparent":"#eff6ff"}}>
        <div style={{display:"flex",justifyContent:"space-between",marginBottom:3}}><span style={{fontSize:11,fontWeight:600,color:n.read?"#737373":"#1a1a1a"}}>{n.title}</span><span style={{fontSize:9,color:"#b3b3b3"}}>{new Date(n.ts).toLocaleString("es-CL",{day:"2-digit",month:"short",hour:"2-digit",minute:"2-digit"})}</span></div>
        <div style={{fontSize:11,color:"#737373",lineHeight:1.35}}>{n.body}</div>
      </div>)}</div>
    </div>}
  </div>);
}
...
function HeatRow({label,tickets,maxCells}){
  const open=tickets.filter(t=>t.stage!=="finalizado");const closed=tickets.filter(t=>t.stage==="finalizado");
  const cells=[];[...open].sort((a,b)=>{const o={Urgente:0,Alta:1,Media:2,Baja:3};return o[a.priority]-o[b.priority]}).forEach(t=>cells.push({color:PRI_CELL[t.priority],tip:"#"+String(t.num).padStart(3,"0")+" "+t.priority+" - "+t.desc.substring(0,30)}));
...
function Dashboard({items,onBack,conn}){
  ...
  const legend=<div style={{display:"flex",gap:8,fontSize:10,color:"#a3a3a3"}}><span style={{display:"flex",alignItems:"center",gap:3}}><span style={{width:8,height:8,borderRadius:1,background:"#dc2626"}}/>Urg</span><span style={{display:"flex",alignItems:"center",gap:3}}><span style={{width:8,height:8,borderRadius:1,background:"#f97316"}}/>Alta</span><span style={{display:"flex",alignItems:"center",gap:3}}><span style={{width:8,height:8,borderRadius:1,background:"#fbbf24"}}/>Med</span><span style={{display:"flex",alignItems:"center",gap:3}}><span style={{width:8,height:8,borderRadius:1,background:"#d4d4d4"}}/>Baja</span><span style={{marginLeft:8,fontFamily:"'JetBrains Mono',monospace",fontWeight:500}}>Tot | Abiertos | Cerrados</span></div>;
  return(<div style={{flex:1,overflowY:"auto",padding:"20px 24px"}}>
    <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:20}}><div style={{display:"flex",alignItems:"center",gap:12}}><button onClick={onBack} style={{...BD,padding:"6px 12px",fontSize:11,display:"flex",alignItems:"center",gap:4}}><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><polyline points="15 18 9 12 15 6"/></svg>Kanban</button><h2 style={{fontSize:16,fontWeight:700,margin:0}}>Dashboard</h2></div><div style={{display:"flex",gap:4}}><button onClick={()=>setTab("sedes")} style={ts(tab==="sedes")}>Sedes</button><button onClick={()=>setTab("tipos")} style={ts(tab==="tipos")}>Tipos</button><button onClick={()=>setTab("proveedores")} style={ts(tab==="proveedores")}>Proveedores</button><button onClick={()=>setTab("usuarios")} style={ts(tab==="usuarios")}>Usuarios</button></div></div>
    <div style={{display:"flex",gap:16,marginBottom:24}}>{[{l:"Total",v:items.length,c:"#1a1a1a"},{l:"Abiertos",v:to,c:"#f97316"},{l:"Cerrados",v:tc,c:"#16a34a"},{l:"Urgentes",v:urg,c:"#dc2626"},{l:"Alta",v:alt,c:"#ea580c"}].map(s=><div key={s.l} style={{background:"#fafafa",border:"1px solid #f0f0f0",borderRadius:8,padding:"12px 16px",flex:1}}><div style={{fontSize:22,fontWeight:700,fontFamily:"'JetBrains Mono',monospace",color:s.c}}>{s.v}</div><div style={{fontSize:10,color:"#a3a3a3",fontWeight:500,textTransform:"uppercase",marginTop:2}}>{s.l}</div></div>)}</div>
...
function Detail({item,onClose,onUpdate,onDelete,saving,user}){
  ...
  const doSave=(id,upd,nt)=>{setSaved(false);onUpdate(id,upd,nt).then(()=>{setSaved(true);setTimeout(()=>setSaved(false),3000)}).catch(()=>{})};
  const hCm=()=>{if(!cm.trim())return;doSave(item.id,{comments:[...(item.comments||[]),{who:user.name,text:cm,date:today()}]},"comment");setCm("")};
  const hDCm=i=>{const c=[...(item.comments||[])];c.splice(i,1);doSave(item.id,{comments:c})};
  const hQ=()=>{if(!pv||!am)return;saveProv(pv);doSave(item.id,{provider:pv,amount:Number(am),stage:"pago"},"stage")};
  ...
  return(
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.15)",zIndex:1000,display:"flex",justifyContent:"flex-end",backdropFilter:"blur(3px)",animation:"fadeIn 0.1s ease"}} onClick={onClose}>
      <div style={{width:500,maxWidth:"94vw",background:"#fff",height:"100vh",overflowY:"auto",borderLeft:"1px solid #e5e5e5",animation:"slideIn 0.15s ease"}} onClick={e=>e.stopPropagation()}>
        {saving&&<div style={{position:"sticky",top:0,left:0,right:0,height:2,background:"#3b82f6",animation:"pulse 1s infinite",zIndex:10}}/>}
        {saved&&!saving&&<div style={{position:"sticky",top:0,left:0,right:0,background:"#f0fdf4",borderBottom:"1px solid #bbf7d0",padding:"6px 16px",display:"flex",alignItems:"center",gap:6,zIndex:10,animation:"fadeIn 0.15s"}}><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#16a34a" strokeWidth="2.5"><polyline points="20 6 9 17 4 12"/></svg><span style={{fontSize:11,color:"#16a34a",fontWeight:500}}>Guardado correctamente</span></div>}
        ...
        <div style={{padding:"16px 24px",borderBottom:"1px solid #f0f0f0",flexShrink:0,display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
          <div><div style={{fontSize:9,fontWeight:500,color:"#b3b3b3",textTransform:"uppercase",marginBottom:2}}>Sede</div><div style={{fontSize:12,fontWeight:500}}>{item.sede}</div></div>
          <div><div style={{fontSize:9,fontWeight:500,color:"#b3b3b3",textTransform:"uppercase",marginBottom:2}}>Solicitante</div>{isAdm?<div style={{display:"flex",gap:4,alignItems:"center"}}><input value={editBy} onChange={e=>setEditBy(e.target.value)} style={{...I,padding:"4px 8px",fontSize:12,width:"auto",flex:1}}/>{editBy!==item.by&&<button onClick={()=>doSave(item.id,{by:editBy})} style={{...BP,padding:"3px 8px",fontSize:10}}>OK</button>}</div>:<div style={{fontSize:12,fontWeight:500}}>{item.by}</div>}</div>
          <div><div style={{fontSize:9,fontWeight:500,color:"#b3b3b3",textTransform:"uppercase",marginBottom:2}}>Fecha</div><div style={{fontSize:12,fontWeight:500}}>{fdate(item.date)+" ("+daysAgo(item.date)+"d)"}</div></div>
          <div><div style={{fontSize:9,fontWeight:500,color:"#b3b3b3",textTransform:"uppercase",marginBottom:2}}>Responsable</div><div style={{display:"flex",gap:4,alignItems:"center"}}>{(isAdm||user.role==="ops")?<><select value={assignee} onChange={e=>setAssignee(e.target.value)} style={{...I,padding:"4px 8px",fontSize:11,flex:1,cursor:"pointer"}}><option value="">Sin asignar</option>{opsUsers.map(u=><option key={u.email} value={u.name}>{u.name}</option>)}</select>{assignee!==(item.assignee||"")&&<button onClick={hAssign} style={{...BP,padding:"3px 8px",fontSize:10}}>OK</button>}</>:<div style={{fontSize:12,fontWeight:500,color:item.assignee?"#3b82f6":"#d4d4d4"}}>{item.assignee||"Sin asignar"}</div>}</div></div>
        </div>

        {item.stage==="requerimiento"&&<div style={{padding:"16px 24px",borderBottom:"1px solid #f0f0f0",background:"#fafafa"}}><div style={SL}>Asignar proveedor y cotizacion</div><div style={{display:"flex",flexDirection:"column",gap:8}}><ProvInput value={pv} onChange={setPv}/><input value={am} onChange={e=>setAm(e.target.value)} placeholder="Monto cotizado (CLP)" type="number" style={I}/><button onClick={hQ} disabled={!pv||!am} style={{...BP,width:"100%",justifyContent:"center",opacity:(!pv||!am)?0.35:1}}>{saving?"Guardando...":"Guardar y enviar a Pago"}</button></div></div>}

        {item.stage==="pago"&&<div style={{padding:"16px 24px",borderBottom:"1px solid #f0f0f0",background:"#fafafa"}}><div style={SL}>Cotizacion</div><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:14}}><div><div style={ML}>Proveedor'{canEdit&&<button onClick={()=>setShowEditProv(!showEditProv)} style={{background:"none",border:"none",cursor:"pointer",color:"#3b82f6",fontSize:9,marginLeft:4}}>{showEditProv?"cancelar":"editar"}</button>}</div>{showEditProv?<div style={{display:"flex",gap:4}}><input value={editProv} onChange={e=>setEditProv(e.target.value)} style={{...I,padding:"4px 8px",fontSize:12,flex:1}}/><button onClick={()=>{if(editProv){saveProv(editProv);doSave(item.id,{provider:editProv});setShowEditProv(false)}}} style={{...BP,padding:"4px 10px",fontSize:10}}>OK</button></div>:<div style={{fontSize:12,fontWeight:500}}>{item.provider}</div>}</div><div><div style={ML}>Monto total{canEdit&&<button onClick={()=>setShowEditAmt(!showEditAmt)} style={{background:"none",border:"none",cursor:"pointer",color:"#3b82f6",fontSize:9,marginLeft:4}}>{showEditAmt?"cancelar":"editar"}</button>}</div>{showEditAmt?<div style={{display:"flex",gap:4}}><input value={editAmt} onChange={e=>setEditAmt(e.target.value)} type="number" style={{...I,padding:"4px 8px",fontSize:14,fontWeight:700,fontFamily:"'JetBrains Mono',monospace",flex:1}}/><button onClick={()=>{if(editAmt){doSave(item.id,{amount:Number(editAmt)});setShowEditAmt(false)}}} style={{...BP,padding:"4px 10px",fontSize:10}}>OK</button></div>:<div style={{fontSize:16,fontWeight:700,fontFamily:"'JetBrains Mono',monospace"}}>{fmt(item.amount)}</div>}</div></div>{cPay?<><div style={SL}>Definir pago</div><div style={{display:"flex",flexDirection:"column",gap:8}}><div style={{display:"flex",alignItems:"center",gap:8}}><div style={{position:"relative",flex:1}}><input value={py} onChange={e=>{const v=e.target.value;if(v===""||(/^\d{0,3}$/.test(v)&&Number(v)<=100))setPy(v)}} placeholder="Ej: 50" style={I}/><span style={{position:"absolute",right:10,top:"50%",transform:"translateY(-50%)",fontSize:12,color:"#a3a3a3",pointerEvents:"none"}}>%</span></div><div style={{display:"flex",gap:4}}>{[0,25,50,100].map(p=><button key={p} onClick={()=>setPy(String(p))} style={{padding:"7px 10px",borderRadius:6,fontSize:11,fontWeight:500,cursor:"pointer",fontFamily:"'Sora',sans-serif",border:py===String(p)?"1.5px solid #1a1a1a":"1px solid #e5e5e5",background:py===String(p)?"#1a1a1a":"#fff",color:py===String(p)?"#fff":"#737373"}}>{p}%</button>)}</div></div>{py!==""&&<div style={{background:"#fff",border:"1px solid #e5e5e5",borderRadius:7,padding:"10px 12px",display:"flex",justifyContent:"space-between"}}><span style={{fontSize:11,color:"#737373"}}>{Number(py)===0?"Pago a 30 dias":"Monto ("+py+"%)"}</span><span style={{fontSize:15,fontWeight:700,fontFamily:"'JetBrains Mono',monospace"}}>{fmt(Math.round(item.amount*Number(py)/100))}</span></div>}<button onClick={hPy} disabled={py===""} style={{...BP,width:"100%",justifyContent:"center",opacity:py===""?0.35:1}}>{saving?"Guardando...":"Aprobar y pasar a En Proceso"}</button></div></>:<div style={{background:"#fefce8",border:"1px solid #fef08a",borderRadius:7,padding:"10px 14px",fontSize:12,color:"#a16207"}}>Esperando aprobacion de Administrador</div>}</div>}

        {item.stage==="en_proceso"&&<div style={{padding:"16px 24px",borderBottom:"1px solid #f0f0f0",background:"#fafafa"}}><div style={SL}>Cotizacion y pago</div><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:6}}><div><div style={ML}>Proveedor'{canEdit&&<button onClick={()=>setShowEditProv(!showEditProv)} style={{background:"none",border:"none",cursor:"pointer",color:"#3b82f6",fontSize:9,marginLeft:4}}>{showEditProv?"cancelar":"editar"}</button>}</div>{showEditProv?<div style={{display:"flex",gap:4}}><input value={editProv} onChange={e=>setEditProv(e.target.value)} style={{...I,padding:"4px 8px",fontSize:12,flex:1}}/><button onClick={()=>{if(editProv){saveProv(editProv);doSave(item.id,{provider:editProv});setShowEditProv(false)}}} style={{...BP,padding:"4px 10px",fontSize:10}}>OK</button></div>:<div style={{fontSize:12,fontWeight:500}}>{item.provider}</div>}</div><div><div style={ML}>Monto total{canEdit&&<button onClick={()=>setShowEditAmt(!showEditAmt)} style={{background:"none",border:"none",cursor:"pointer",color:"#3b82f6",fontSize:9,marginLeft:4}}>{showEditAmt?"cancelar":"editar"}</button>}</div>{showEditAmt?<div style={{display:"flex",gap:4}}><input value={editAmt} onChange={e=>setEditAmt(e.target.value)} type="number" style={{...I,padding:"4px 8px",fontSize:14,fontWeight:700,fontFamily:"'JetBrains Mono',monospace",flex:1}}/><button onClick={()=>{if(editAmt){doSave(item.id,{amount:Number(editAmt)});setShowEditAmt(false)}}} style={{...BP,padding:"4px 10px",fontSize:10}}>OK</button></div>:<div style={{fontSize:14,fontWeight:700,fontFamily:"'JetBrains Mono',monospace"}}>{fmt(item.amount)}</div>}</div></div><div style={{background:"#fff",border:"1px solid #e5e5e5",borderRadius:7,padding:"8px 12px",display:"flex",justifyContent:"space-between",marginBottom:10}}><span style={{fontSize:11,color:"#737373"}}>Pagado ({item.payment}%)</span><span style={{fontSize:14,fontWeight:700,fontFamily:"'JetBrains Mono',monospace"}}>{fmt(Math.round(item.amount*Number(item.payment)/100))}</span></div><div style={SL}>Fecha de ejecucion</div><div style={{display:"flex",gap:6,marginBottom:14}}><input type="date" value={ed} onChange={e=>setEd(e.target.value)} style={{...I,flex:1}}/><button onClick={hEx} disabled={!ed} style={{...BP,padding:"8px 14px",opacity:ed?1:0.35}}>{item.execDate?"Actualizar":"Guardar"}</button></div>{item.execDate&&<div style={{background:"#eff6ff",border:"1px solid #bfdbfe",borderRadius:7,padding:"8px 12px",fontSize:12,color:"#2563eb",marginBottom:14}}>{"Programado: "+fdate(item.execDate)}</div>}{cFin?<button onClick={hFn} style={{...BP,width:"100%",justifyContent:"center",background:"#16a34a",border:"none"}}>{saving?"Guardando...":"Marcar como Finalizado"}</button>:<div style={{background:"#fefce8",border:"1px solid #fef08a",borderRadius:7,padding:"10px 14px",fontSize:12,color:"#a16207"}}>Solo Operaciones puede finalizar</div>}</div>}

        {item.stage==="finalizado"&&<div style={{padding:"16px 24px",borderBottom:"1px solid #f0f0f0"}}><div style={SL}>Resumen</div><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:6}}><div><div style={ML}>Proveedor'{canEdit&&<button onClick={()=>setShowEditProv(!showEditProv)} style={{background:"none",border:"none",cursor:"pointer",color:"#3b82f6",fontSize:9,marginLeft:4}}>{showEditProv?"cancelar":"editar"}</button>}</div>{showEditProv?<div style={{display:"flex",gap:4}}><input value={editProv} onChange={e=>setEditProv(e.target.value)} style={{...I,padding:"4px 8px",fontSize:12,flex:1}}/><button onClick={()=>{if(editProv){saveProv(editProv);doSave(item.id,{provider:editProv});setShowEditProv(false)}}} style={{...BP,padding:"4px 10px",fontSize:10}}>OK</button></div>:<div style={{fontSize:12,fontWeight:500}}>{item.provider}</div>}</div><div><div style={ML}>Monto total{canEdit&&<button onClick={()=>setShowEditAmt(!showEditAmt)} style={{background:"none",border:"none",cursor:"pointer",color:"#3b82f6",fontSize:9,marginLeft:4}}>{showEditAmt?"cancelar":"editar"}</button>}</div>{showEditAmt?<div style={{display:"flex",gap:4}}><input value={editAmt} onChange={e=>setEditAmt(e.target.value)} type="number" style={{...I,padding:"4px 8px",fontSize:14,fontWeight:700,fontFamily:"'JetBrains Mono',monospace",flex:1}}/><button onClick={()=>{if(editAmt){doSave(item.id,{amount:Number(editAmt)});setShowEditAmt(false)}}} style={{...BP,padding:"4px 10px",fontSize:10}}>OK</button></div>:<div style={{fontSize:14,fontWeight:700,fontFamily:"'JetBrains Mono',monospace"}}>{fmt(item.amount)}</div>}</div></div>{item.payment&&item.amount&&<div style={{background:"#f0fdf4",border:"1px solid #bbf7d0",borderRadius:7,padding:"8px 12px",display:"flex",justifyContent:"space-between",marginBottom:8}}><span style={{fontSize:11,color:"#16a34a"}}>Pagado ({item.payment}%)</span><span style={{fontSize:14,fontWeight:700,fontFamily:"'JetBrains Mono',monospace",color:"#16a34a"}}>{fmt(Math.round(item.amount*Number(item.payment)/100))}</span></div>}{item.closedAt&&<div style={{display:"flex",alignItems:"center",gap:6,color:"#16a34a",fontSize:12,fontWeight:500,background:"#f0fdf4",padding:"8px 12px",borderRadius:6,marginTop:4}}>{"Cerrado el "+fdate(item.closedAt)}</div>}</div>}

        <div style={{marginTop:"auto"}}><div style={{padding:"12px 24px 6px",flexShrink:0}}><div style={SL}>{"Actividad ("+(item.comments||[]).length+")"}</div></div><div style={{flex:1,overflowY:"auto",padding:"0 24px"}}>{(item.comments||[]).length===0&&<div style={{padding:"16px 0",color:"#e0e0e0",fontSize:11,textAlign:"center"}}>Sin actividad</div>}{(item.comments||[]).map((c,i)=><div key={i} style={{padding:"8px 0",borderBottom:"1px solid #f5f5f5",display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}><div><div style={{display:"flex",alignItems:"center",gap:5,marginBottom:2}}><span style={{fontSize:11,fontWeight:600}}>{c.who}</span><span style={{fontSize:9,color:"#b3b3b3"}}>{fdate(c.date)}</span></div><div style={{fontSize:12,color:"#525252",lineHeight:1.35}}>{c.text}</div></div><button onClick={()=>hDCm(i)} style={{background:"none",border:"none",cursor:"pointer",color:"#e0e0e0",padding:2,flexShrink:0}}><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg></button></div>)}</div>
...
function NewModal({onClose,onSubmit,saving,user}){
  const[f,setF]=useState({category:"",sede:"",priority:"Baja",desc:"",by:user.name});const set=(k,v)=>setF({...f,[k]:v});const ok=f.category&&f.sede&&f.desc&&f.by;
  return(<div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.15)",zIndex:1000,display:"flex",alignItems:"center",justifyContent:"center",backdropFilter:"blur(4px)",animation:"fadeIn 0.1s ease"}} onClick={onClose}><div style={{background:"#fff",borderRadius:12,width:440,maxWidth:"92vw",boxShadow:"0 16px 32px rgba(0,0,0,0.08)",animation:"slideUp 0.15s ease"}} onClick={e=>e.stopPropagation()}>
    ...
    <div style={{padding:"20px 24px",display:"flex",flexDirection:"column",gap:14}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}><div><label style={FL}>Sede</label><SedeSelect value={f.sede} onChange={v=>set("sede",v)}/></div><div><label style={FL}>Prioridad</label>{user.role==="admin"?<select value={f.priority} onChange={e=>set("priority",e.target.value)} style={{...I,cursor:"pointer"}}>{Object.keys(PRIORITY).map(p=><option key={p} value={p}>{p}</option>)}</select>:<div style={{...I,background:"#fafafa",color:"#a3a3a3"}}>Baja</div>}</div></div>
...
export default function App(){
  ...
  const doNotify=async(item,type)=>{
    if(!conn)return;const tag="#"+String(item.num).padStart(3,"0");const allU=getAllUsers();
    const opsEmails=allU.filter(u=>u.role==="ops").map(u=>u.email);
    const admEmails=allU.filter(u=>u.role==="admin").map(u=>u.email);
    let emailTo=[...new Set([...admEmails,...opsEmails])];
    ...
    if(emailTo.length)try{await sendNotification(emailTo,sub,body)}catch(e){console.error("Notify:",e)}
  };
  ...
  const handleDel=async id=>{setItems(p=>p.filter(t=>t.id!==id));setSel(null);if(conn){savRef.current=true;setSaving(true);try{await deleteTicket(id)}catch(e){console.error(e)}finally{setSaving(false);savRef.current=false}}};

  if(!user)return<LoginScreen onLogin={setUser}/>;
  const filt=items.filter(t=>{if(search){const s=search.toLowerCase();if(!t.desc.toLowerCase().includes(s)&&!t.sede.toLowerCase().includes(s)&&!(t.provider||"").toLowerCase().includes(s)&&!String(t.num).padStart(3,"0").includes(s)&&!(t.assignee||"").toLowerCase().includes(s))return false}if(fCat&&t.category!==fCat)return false;if(fPri&&t.priority!==fPri)return false;return true});
