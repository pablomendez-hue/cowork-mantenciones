...
import { COLUMNS, COL_IDS, CATEGORIES, PRIORITY, SEDES, fmt, fdate, daysAgo, today } from "./constants.js";
import { isConfigured, fetchAllTickets, createTicket, updateTicket, testConnection } from "./sheets.js";

const DEMO_DATA = [
  { id:1, category:"Servicio Adicional", desc:"Instalaci\u00f3n de 12 tomacorrientes oficina piso 7", sede:"Suecia", priority:"Alta", stage:"finalizado", by:"Mar\u00eda L\u00f3pez", date:"2026-01-21", provider:"Servicios e Ingenier\u00eda IA SPA", amount:642600, payment:"100", closedAt:"2026-02-01", comments:[] },
  { id:2, category:"Mantenci\u00f3n", desc:"Cambio rejilla desag\u00fce lavaplatos", sede:"SalesForce", priority:"Urgente", stage:"en_proceso", by:"Carlos D\u00edaz", date:"2025-12-03", provider:"Servicios e Ingenier\u00eda IA SPA", amount:95200, payment:"50", closedAt:null, comments:[] },
  { id:3, category:"Servicio Adicional", desc:"Instalaci\u00f3n tope puerta oficinas piso 3", sede:"Abedules", priority:"Baja", stage:"pago", by:"Ana Torres", date:"2026-02-12", provider:"Servicios e Ingenier\u00eda IA", amount:71400, payment:null, closedAt:null, comments:[] },
  { id:4, category:"Aire Acondicionado", desc:"Revisi\u00f3n y recarga gas split oficina 301", sede:"Plaza Ega\u00f1a", priority:"Media", stage:"requerimiento", by:"Mar\u00eda L\u00f3pez", date:"2026-02-17", provider:null, amount:null, payment:null, closedAt:null, comments:[] },
  { id:5, category:"Mantenci\u00f3n", desc:"Traslado mesa sala reuniones desde Londres", sede:"Londres", priority:"Media", stage:"pago", by:"Carlos D\u00edaz", date:"2026-02-03", provider:"Edgar Moreno", amount:75000, payment:null, closedAt:null, comments:[] },
];
...
function SedeSelector({value,onChange}){
  ...
  if(isOtro){return(
    <div style={{display:"flex",gap:4}}>
      <input value={custom} onChange={e=>setCustom(e.target.value)} placeholder="Nombre sede..." style={{...inputStyle,flex:1}} onKeyDown={e=>e.key==="Enter"&&handleSaveCustom()} autoFocus/>
      <button onClick={handleSaveCustom} disabled={!custom.trim()} style={{...btnPrimary,padding:"8px 10px",opacity:custom.trim()?1:0.35}}>OK</button>
      <button onClick={()=>{setIsOtro(false);setCustom("")}} style={{...btnDefault,padding:"8px 10px"}}>X</button>
    </div>
  )}
  return(<select value={value} onChange={handleSelect} style={{...inputStyle,cursor:"pointer"}}><option value="">Seleccionar...</option>{sedes.map(s=><option key={s} value={s}>{s}</option>)}<option value="__otro__">+ Otra sede...</option></select>);
}
...
function ProviderInput({value,onChange}){
  ...
  return(
    <div ref={ref} style={{position:"relative"}}>
      <input value={value} onChange={e=>{onChange(e.target.value);setShowList(true)}} onFocus={()=>setShowList(true)} placeholder="Nombre del proveedor" style={inputStyle}/>
      {showList&&filtered.length>0&&(
        <div style={{position:"absolute",top:"100%",left:0,right:0,marginTop:2,background:"#fff",border:"1px solid #e5e5e5",borderRadius:7,boxShadow:"0 8px 24px rgba(0,0,0,0.08)",zIndex:20,maxHeight:160,overflowY:"auto"}}>
          {filtered.map(p=>(<div key={p} onClick={()=>{onChange(p);setShowList(false)}} style={{padding:"8px 12px",fontSize:12,cursor:"pointer"}} onMouseEnter={e=>e.currentTarget.style.background="#f5f5f5"} onMouseLeave={e=>e.currentTarget.style.background="transparent"}>{p}</div>))}
        </div>
      )}
    </div>
  );
    ...
    <div draggable onDragStart={(e)=>{e.dataTransfer.setData("text/plain",String(item.id));e.dataTransfer.effectAllowed="move";onDragStart(item.id)}} onClick={()=>onOpen(item)}
      ...
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:5}}>
        <span style={{fontSize:9,fontWeight:600,letterSpacing:"0.06em",textTransform:"uppercase",color:cat.color,background:cat.bg,border:"1px solid "+cat.border,padding:"1px 6px",borderRadius:3}}>
          {item.category==="Aire Acondicionado"?"A/C":item.category==="Servicio Adicional"?"Serv. Adic.":"MNT"}
        </span>
  ...
  return(
    <div style={{flex:1,minWidth:240,maxWidth:340,display:"flex",flexDirection:"column",height:"100%"}}
      ...
      <div style={{padding:"0 2px 10px",flexShrink:0}}>
        <div style={{display:"flex",alignItems:"center",gap:6}}>
          <span style={{fontSize:15}}>{col.icon}</span><span style={{fontSize:13,fontWeight:600}}>{col.label}</span>
...
function DetailPanel({item,onClose,onUpdate,saving}){
  ...
  const handleComment=()=>{if(!comment.trim())return;onUpdate(item.id,{comments:[...(item.comments||[]),{who:"Tu",text:comment,date:today()}]});setComment("")};
      ...
      <div style={{width:500,maxWidth:"94vw",background:"#fff",height:"100vh",overflowY:"auto",borderLeft:"1px solid #e5e5e5",animation:"slideIn 0.15s ease",display:"flex",flexDirection:"column"}} onClick={e=>e.stopPropagation()}>
        ...
        {item.stage==="requerimiento"&&(
          <div style={{padding:"16px 24px",borderBottom:"1px solid #f0f0f0",background:"#fafafa",flexShrink:0}}>
            <div style={sectionLabel}>Asignar proveedor y cotizacion</div>
            <div style={{display:"flex",flexDirection:"column",gap:8}}>
              <ProviderInput value={provider} onChange={setProvider}/>
              <input value={amount} onChange={e=>setAmount(e.target.value)} placeholder="Monto cotizado (CLP)" type="number" style={inputStyle}/>
        ...
        {item.stage==="pago"&&(
          <div style={{padding:"16px 24px",borderBottom:"1px solid #f0f0f0",background:"#fafafa",flexShrink:0}}>
            <div style={sectionLabel}>Cotizacion</div>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:14}}>
              <div><div style={metaLabel}>Proveedor</div><div style={{fontSize:12,fontWeight:500}}>{item.provider}</div></div>
              <div><div style={metaLabel}>Monto total</div><div style={{fontSize:16,fontWeight:700,fontFamily:"'JetBrains Mono',monospace"}}>{fmt(item.amount)}</div></div>
            </div>
            <div style={sectionLabel}>Definir pago</div>
            <div style={{display:"flex",flexDirection:"column",gap:8}}>
              <div style={{display:"flex",alignItems:"center",gap:8}}>
                <div style={{position:"relative",flex:1}}>
                  <input value={payment} onChange={e=>{const v=e.target.value;if(v===""||(/^\d{0,3}$/.test(v)&&Number(v)<=100))setPayment(v)}} placeholder="Ej: 50" type="text" inputMode="numeric" style={inputStyle}/>
                  <span style={{position:"absolute",right:10,top:"50%",transform:"translateY(-50%)",fontSize:12,color:"#a3a3a3",pointerEvents:"none"}}>%</span>
                </div>
                <div style={{display:"flex",gap:4}}>
                  {[25,50,100].map(p=>(<button key={p} onClick={()=>setPayment(String(p))} style={{padding:"7px 10px",borderRadius:6,fontSize:11,fontWeight:500,cursor:"pointer",fontFamily:"'Sora',sans-serif",border:payment===String(p)?"1.5px solid #1a1a1a":"1px solid #e5e5e5",background:payment===String(p)?"#1a1a1a":"#fff",color:payment===String(p)?"#fff":"#737373",transition:"all 0.12s"}}>{p}%</button>))}
                </div>
              </div>
              {payment&&Number(payment)>0&&(
                <div style={{background:"#fff",border:"1px solid #e5e5e5",borderRadius:7,padding:"10px 12px",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                  <span style={{fontSize:11,color:"#737373"}}>Monto a pagar ({payment}%)</span>
                  <span style={{fontSize:15,fontWeight:700,fontFamily:"'JetBrains Mono',monospace",color:"#1a1a1a"}}>{fmt(Math.round(item.amount*Number(payment)/100))}</span>
                </div>
              )}
              <button onClick={handleSavePayment} disabled={!payment||Number(payment)<=0} style={{...btnPrimary,width:"100%",justifyContent:"center",opacity:(!payment||Number(payment)<=0)?0.35:1,cursor:(!payment||Number(payment)<=0)?"not-allowed":"pointer"}}>
        ...
        {item.stage==="en_proceso"&&(
          <div style={{padding:"16px 24px",borderBottom:"1px solid #f0f0f0",background:"#fafafa",flexShrink:0}}>
            <div style={sectionLabel}>Cotizacion y pago</div>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:6}}>
              <div><div style={metaLabel}>Proveedor</div><div style={{fontSize:12,fontWeight:500}}>{item.provider}</div></div>
              <div><div style={metaLabel}>Monto total</div><div style={{fontSize:14,fontWeight:700,fontFamily:"'JetBrains Mono',monospace"}}>{fmt(item.amount)}</div></div>
            </div>
            <div style={{background:"#fff",border:"1px solid #e5e5e5",borderRadius:7,padding:"8px 12px",display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}>
        ...
        {item.stage==="finalizado"&&(
          <div style={{padding:"16px 24px",borderBottom:"1px solid #f0f0f0",flexShrink:0}}>
            ...
            {item.payment&&item.amount&&(
              <div style={{background:"#f0fdf4",border:"1px solid #bbf7d0",borderRadius:7,padding:"8px 12px",display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
                <span style={{fontSize:11,color:"#16a34a"}}>Pagado ({item.payment}%)</span>
                <span style={{fontSize:14,fontWeight:700,fontFamily:"'JetBrains Mono',monospace",color:"#16a34a"}}>{fmt(Math.round(item.amount*Number(item.payment)/100))}</span>
              </div>
            )}
            {item.closedAt&&(
              <div style={{display:"flex",alignItems:"center",gap:6,color:"#16a34a",fontSize:12,fontWeight:500,background:"#f0fdf4",padding:"8px 12px",borderRadius:6,marginTop:8}}>
                ...
                {"Cerrado el "+fdate(item.closedAt)+" - "+Math.max(0,daysAgo(item.date)-daysAgo(item.closedAt))+" dias"}
              </div>
            )}
          </div>
        )}
        ...
        <div style={{flex:1,display:"flex",flexDirection:"column",minHeight:0}}>
          ...
          <div style={{flex:1,overflowY:"auto",padding:"0 24px"}}>
            ...
            {(item.comments||[]).map((c,i)=>(
              <div key={i} style={{padding:"8px 0",borderBottom:"1px solid #f5f5f5"}}>
                <div style={{display:"flex",alignItems:"center",gap:5,marginBottom:2}}><span style={{fontSize:11,fontWeight:600}}>{c.who}</span><span style={{fontSize:9,color:"#b3b3b3"}}>{fdate(c.date)}</span></div>
          ...
          <div style={{padding:"12px 24px",borderTop:"1px solid #f0f0f0",display:"flex",gap:6,flexShrink:0}}>
            <input value={comment} onChange={e=>setComment(e.target.value)} placeholder="Comentario..." style={{...inputStyle,flex:1,padding:"7px 10px",fontSize:12}} onKeyDown={e=>e.key==="Enter"&&handleComment()}/>
    ...
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.15)",zIndex:1000,display:"flex",alignItems:"center",justifyContent:"center",backdropFilter:"blur(4px)",animation:"fadeIn 0.1s ease"}} onClick={onClose}>
      <div style={{background:"#fff",borderRadius:12,width:440,maxWidth:"92vw",boxShadow:"0 16px 32px rgba(0,0,0,0.08)",animation:"slideUp 0.15s ease"}} onClick={e=>e.stopPropagation()}>
        <div style={{padding:"20px 24px 0",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
          ...
          <button onClick={onClose} style={{background:"none",border:"none",cursor:"pointer",color:"#b3b3b3"}}><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg></button>
        </div>
        <div style={{padding:"20px 24px",display:"flex",flexDirection:"column",gap:14}}>
          <div><label style={formLabel}>Solicitante</label><input value={f.by} onChange={e=>set("by",e.target.value)} placeholder="Tu nombre" style={inputStyle}/></div>
          <div><label style={formLabel}>Tipo</label>
            <div style={{display:"flex",gap:6}}>
              {Object.entries(CATEGORIES).map(([name,c])=>(
                <button key={name} onClick={()=>set("category",name)} style={{flex:1,padding:"8px 6px",borderRadius:6,fontSize:11,fontWeight:500,cursor:"pointer",fontFamily:"inherit",textAlign:"center",border:f.category===name?"2px solid "+c.color:"1px solid #e5e5e5",background:f.category===name?c.bg:"#fff",color:f.category===name?c.color:"#999",transition:"all 0.12s"}}>
                  {name==="Aire Acondicionado"?"A/C":name==="Servicio Adicional"?"Serv. Adicional":name}
                </button>
              ))}
            </div>
          </div>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
            <div><label style={formLabel}>Sede</label><SedeSelector value={f.sede} onChange={(v)=>set("sede",v)}/></div>
            <div><label style={formLabel}>Prioridad</label><select value={f.priority} onChange={e=>set("priority",e.target.value)} style={{...inputStyle,cursor:"pointer"}}>{Object.keys(PRIORITY).map(p=><option key={p} value={p}>{p}</option>)}</select></div>
          </div>
          <div><label style={formLabel}>Descripcion</label><textarea value={f.desc} onChange={e=>set("desc",e.target.value)} placeholder="Describe el requerimiento..." rows={2} style={{...inputStyle,resize:"vertical",lineHeight:1.4}}/></div>
        </div>
        <div style={{padding:"10px 24px 20px",display:"flex",justifyContent:"flex-end",gap:6}}>
          ...
          <button onClick={()=>{if(ok)onSubmit(f)}} disabled={!ok||saving} style={{...btnPrimary,opacity:(ok&&!saving)?1:0.35,cursor:(ok&&!saving)?"pointer":"not-allowed"}}>{saving?"Guardando...":"Crear"}</button>
        </div>
      </div>
    </div>
  );
...
export default function App(){
  ...
  const handleDrop=useCallback((id,newStage)=>{
    setItems(prev=>prev.map(it=>{
      ...
      const from=COL_IDS.indexOf(it.stage);const to=COL_IDS.indexOf(newStage);
  ...
  const handleNew=async(f)=>{
    const item={id:Date.now(),...f,stage:"requerimiento",date:today(),provider:null,amount:null,payment:null,closedAt:null,comments:[]};
  ...
  const handleUpdate=async(id,updates)=>{
    ...
    if(updatedItem)await persist(updatedItem);
  };
      ...
      <div style={{padding:"12px 24px",borderBottom:"1px solid #f0f0f0",display:"flex",alignItems:"center",justifyContent:"space-between",flexShrink:0,background:"#fff",position:"sticky",top:0,zIndex:50}}>
        <div style={{display:"flex",alignItems:"center",gap:16}}>
          ...
          <div style={{display:"flex",alignItems:"center",gap:4,padding:"3px 8px",borderRadius:99,background:connected?"#f0fdf4":"#fefce8",border:connected?"1px solid #bbf7d0":"1px solid #fef08a"}}>
            ...
            <span style={{fontSize:9,fontWeight:500,color:connected?"#16a34a":"#a16207"}}>{connected?"Google Sheets":"Demo"}</span>
          </div>
        ...
        <div style={{display:"flex",alignItems:"center",gap:8}}>
          <div style={{position:"relative"}}>
            ...
            <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Buscar..." style={{...inputStyle,paddingLeft:28,width:170,padding:"6px 10px 6px 28px",fontSize:11}}/>
          </div>
          ...
          <button onClick={()=>setShowNew(true)} style={{...btnPrimary,padding:"6px 14px",fontSize:11}}>
            <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="#fff" strokeWidth="2.5"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg> Nuevo
          </button>
        </div>
      </div>
